...
const twentyCustomers =
  Array.from("0123456789ABCDEFGHIJ", id => ({ id })
  );
describe("CustomerSearch", () => {
  ...
  it("disables the previous button on the first page", async () => {
    ...
    await renderAndWait(<CustomerSearch />);
    expect(buttonWithLabel("Previous").disabled).toBeTruthy();
    await clickAndWait(buttonWithLabel("Next"));
    await clickAndWait(buttonWithLabel("Previous"));
    expect(buttonWithLabel("Previous").disabled).toBeTruthy();
  })

  it("enables previous button once next is clicked and disables again on the first page", async () => {
    global.fetch.mockResolvedValue(
      fetchResponseOk(tenCustomers)
    );
    await renderAndWait(<CustomerSearch />);
    await clickAndWait(buttonWithLabel("Next"));
    expect(buttonWithLabel("Previous").getAttribute("disabled")).toBeNull();
    await clickAndWait(buttonWithLabel("Previous"));
    expect(buttonWithLabel("Previous").getAttribute("disabled")).not.toBeNull();
  })

  it("disables the next button if current listing has < 10 records", async () => {
    global.fetch
      .mockResolvedValueOnce(
        fetchResponseOk(tenCustomers)
      )
    ...
    await renderAndWait(<CustomerSearch />);
    await clickAndWait(buttonWithLabel("Next"));
    expect(buttonWithLabel("Next").disabled).toBeTruthy();
  })

  it("disables the next button if current listing has < 10 records on the first page", async () => {
    global.fetch
      .mockResolvedValue(
        fetchResponseOk(twoCustomers)
      );
